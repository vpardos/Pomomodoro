...
export function useNotifications() {
  const [soundEnabled, setSoundEnabled] = useState(() => loadBool(SOUND_KEY, true));
  ...
  useEffect(() => {
    const checkAlarms = () => {
      ...
      const matchingAlarms = alarms.filter((alarm) => alarm.enabled && alarm.time === currentTime);
      ...
      if (matchingAlarms.length > 0) {
        ...
        const title = labels.length === 1 ? labels[0] : `${labels.length} alarms`;
